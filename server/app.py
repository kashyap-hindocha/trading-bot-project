...
# ═══════════════════════════════════════════
# WebSocket Events & Background Updates
# ═══════════════════════════════════════════
...
def emit_all_data():
    """Emit all dashboard data to connected clients"""
    try:
        # Gather all data
        ...
        socketio.emit('equity_update', equity_data)
        socketio.emit('logs_update', logs_data)
        
    except Exception as e:
        app.logger.error(f'Error emitting data: {e}')

def get_status_data():
    """Get status data (balance, open trades, bot status)"""
    balance = 0.0
    balance_currency = "INR"
    num_trades = 0
    bot_running = False
...
def get_equity_data():
    ...
    try:
        return db.get_equity_history(limit=200)
    except Exception:
        return []
...
def background_updater():
    """Background thread to push updates every 5 seconds"""
    while True:
        time.sleep(5)
        try:
            emit_all_data()
        except Exception as e:
            app.logger.error(f'Background update error: {e}')
